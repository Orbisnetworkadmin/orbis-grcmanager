(ns orbis-grcmanager.routes.services.auth
  (:require [orbis-grcmanager.config :refer [env]]
...
(handler register! [user]
  (if-let [errors (v/validate-create-user user)]
    (do
      (log/error "Error creando el usuario:" errors)
      (bad-request {:error "usuario inválido"}))
    (db/insert-user<!
      (-> user
          (dissoc :pass-confirm)
...
(handler update-user! [{:keys [pass] :as user}]
  (if-let [errors (v/validate-update-user user)]
    (do
      (log/error "Error actualizando el usuario:" errors)
      (bad-request {:error "Usuario inválido"}))
    (ok
      {:user
       (if pass
